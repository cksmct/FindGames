...
| **`googleplay`（Android）** | 8 个分类热门榜 × 8 个国家 | 216~442 个 | 每轮 4 组，单页 1.7~2.6MB |
...
> （Play 不提供、itch/Poki 无接口）→「竞争」项无从推断 → 在推荐页表现为「可评估 0 个 / 竞争未测」，
> 必须人点「查竞争（SERP）」补判断。**iOS 有 `releaseDate`，所以能正常算竞争与新鲜度** ——
> 这是平台能力差异，不是 bug。
...
| 好评 < 60% | ❌ 游戏本身在流失玩家 |
| 已配人工判断且长尾开放度 ≤ 2 | ❌ 竞争饱和 |
| **没有官方上线数据、也没人工竞争判断** | ⚠️ **竞争未测（不给分）** —— 点卡片上的「查竞争（SERP）」补齐 |
