...
| 页面 | 分数 | 明细里有什么 |
|---|---|---|
| 🎮 新游戏雷达 | 雷达分（`score`，验证优先级） | 逐项：搜索量 → `log₂(量/1000)×2` · 涨幅% ÷100 · 起飞档（hype）· 识别权重 ×2 · 人工加分；同时给出**原始输入**（量 / 涨幅 / hype / 权重） |
...
#### 🧩 雷达分五项各是什么（2026-09-26 补：用户问“识别权重”与“人工加分”）

🛑 先说清用途：雷达分是**验证优先级**（先验证谁），不是可做性。所以它的量纲是 0~50 上下，
和「🎯 建站推荐」的 0~100 **不可比**。五项（单一事实源：`src/lib/detect.mjs` 的 `scoreBreakdown`）：

| 项 | 怎么算 | 数据从哪来 | 现状 / 你能怎么用 |
|---|---|---|---|
| 搜索量 | `log₂(量/1000)×2`（2千≈2 · 2万≈8.6 · 20万≈15 · 200万≈22）；**<1 千 = 0，不倒扣** | Google Trends 的 vol | 热搜型条目的主项；来源型条目**天然为 0**（没走 Trends） |
| 涨幅 | `涨幅% ÷ 100`（+100% 加 1 分、+1000% 加 10 分） | Trends 的 growth | 刻意排在权重之后：低量噪音的涨幅往往更高 |
| 起飞档 | 7 天曲线「后段 ÷ 前段」→ ≥99 → 8 · ≥3 → 5 · ≥1.5 → 2 · 其余 0 | 我们自己取的 7 天曲线（`hype`） | 只有取过曲线的条目才有 |
| **识别权重 ×2** | 三条“它是不是游戏”的证据：**Google 分类=Games +2** · **游戏平台词 +2**（roblox/minecraft/steam/xbox/gta…）· **游戏意图词 +1**（gameplay/codes/tier list/update/trailer/beta…）→ 权重 0~5，再 ×2 = **0~10** | 采集时 `gameCandidate()` 判定 | 它是**置信度**，不是热度：**权重 ≥3 = 强信号**（早期真游戏都在 3），人名类噪音全在 2。**来源型条目（Roblox/Steam/itch/App Store 目录）固定 = 3** —— 来源本身就是游戏目录，`name”显然是游戏，所以 ×2 = 6 分 |
| **人工加分** | `config.json` 的 `feedback.boost` 列表命中 **+6**（`FEEDBACK_BOOST_PTS`）；`feedback.block` = **永久否决**（不进雷达、重筛时清掉） | **你自己手写**的配置 | 这是雷达分里**唯一的人工入口**：你认为值得做的词加进去，它下一轮就 +6；线上默认是空的，所以现在全是 0 |

> ⚠️ 顺带一个如实记录（来源：2026-09-26 用户截图 `tabby bombardier 3D` score 6 = 0+0+0+6+0）：
> **来源型条目的雷达分几乎恒在 6~14**（搜索量 / 涨幅天然为 0，只有 权重 6 + 可能的起飞档 8），
> 而来源型占全部条目的 **79%**（2217 条里 2141 条）—— 也就是“雷达页按分数排”对这批**没有区分度**（一片 6 分）。
> 想让它们也有区分度，得把官方数据（访问 / 在线 / 评分人数）折成一项进雷达分 —— 尚未做，等你定。
...
| **内容面** | 14 | 由 genres 推断：图鉴/养成类（RPG / monster catching / Turn Based / Open World）92 · 模拟经营类 70 · Action/Shooter 52 · Escape/Obby/RNG 28 |
| **社区地基** | 20 | Discord +8 · YouTube +6 · Roblox 群组 +6（封顶 20）。🛑 原始分是 0~20 而非 0~100，这是既有口径 |
| 🆕 **竞争饱和度** | 20 | **前十的「专用站」数**（域名含游戏名 = 用户说的"对手当然是新建的站"；通用媒体不算。与「建站推荐」的竞争项**共用同一张分档表**）：0 个=100 · 1~2 个=75 · 3~4 个=50 · 5~7 个=25 · ≥8 个=10 |

档位：`≥75 值得潜伏` · `55~74 观察` · `<55 暂不`；**风险覆盖**（优先级高于分数）：
**前十专用站 ≥5（`taken`「竞争已起」）** · 状态含 Delayed / Maybe Cancelled → 「风险」 · 距发售 ≤7 天 → 「窗口已过」。
