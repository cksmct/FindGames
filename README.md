...
| 约束 | 原因 |
|---|---|
| **按需刷新**（`statsRefreshDays: 7`） | 访问量是"竞争强度"的代理指标，变化慢；每小时刷 20+ 次是浪费配额 |
...
| 项 | 权重 | 怎么算 |
|---|---|---|
| **竞争余量** | 28 | 以 **1000 万访问为峰值**，每偏离一个数量级扣 45 分 |
| **内容面** | 26 | 已挖到的攻略词数量 ≈ 可直接做的页面数 |
| **需求动能** | 18 | 7 天曲线后半段 vs 前半段 |
| **口碑** | 16 | 50% 好评 → 0 分；95% → 100 分 |
| **新鲜度** | 12 | 刚发现 100 分，每多 1 天减 3.3 分 |

**竞争余量是本页最反直觉的一项 —— 它是「中间高、两端低」，不是单调的：**

```
访问      1.0M →  55        访问    100.0M →  55
访问     10.0M → 100  ← 峰值  访问   1000.0M →  10
访问    100.0K →  10        访问  10000.0M →   0
```

依据是实测的访问量量级分布（28 条 Roblox 来源）：`>10B 7 个 · 1B-10B 12 个 · 100M-1B 3 个 · 1M-100M 6 个`。
**可做区间是 1M~100M** —— 太低没有搜索需求，太高没有排名余地。

#### 三条硬否决（与 `roblox-game-breakout-scanner` 技能同一套口径）
...
注意**否决线要和竞争余量曲线对齐**：竞争余量在 1e9 处正好降到 0，所以否决线就定在 1e9。
如果定得更高，会出现「竞争余量已经是 0、却还标着可小试」的自相矛盾（实测踩过：Animal Hospital 23 亿访问拿了 59 分）。

#### 人工竞争判断（`games.competition`）—— 无法自动获取的那一半

**长尾是否被专用 wiki / 专业站占据，自动抓不到，只能人去查。** 但它恰恰最能翻转结论：
...
- `open`（1~5）：长尾开放度。1 = 被单一权威垄断 → **总分 ×0.35**；5 = 蓝海 → ×1.35
- `lagHours`：头部攻略站对最近事件发稿的滞后小时数。≤12h → ×0.6；≥72h → ×1.1

它是**乘数，不参与加权平均** —— 这是否决性信息（"这个长尾已经被占满了"），不是一个能被其它项摊平的分数。

实测效果（Slayers 2）：自动分 **96 → ×0.21 → 20**，结论从「值得做」变成「竞争饱和（人工判断）」，
与手工查证结果一致。**没有这个入口，第一个推荐就是错的。**
...
```
结论            分数   游戏                     访问     攻略词
值得做           90   Ride A Pet             56.9M      9
值得做           80   The Hunt: Roblox 20   167.8M     11   ⚠️限时活动
可小试           63   Kick an Uma             2.4M      4
竞争饱和(人工)     20   Slayers 2              20.7M     12   🔧人工×0.21
缺官方数据         —   gta v / fifa 27 / …              （非 Roblox 来源，判断不了）
巨头级         22~67   Steal a Brainrot 等 19 个
```

**两条诚实的边界**：

1. **27/55 条「缺官方数据」给不出分** —— 它们不是 Roblox 来源，没有访问量参照，**判断不了竞争强度就不该给分**。
   第一版曾把缺失项的权重让给其它项，结果 `gta 6` / `fifa 27` 靠"内容面 + 动能 + 新鲜度"凑出 **99 分排在第一位**。
   **缺失不等于满分，也不等于 0，而是"判断不了"。**
2. **评分不区分「持久需求」和「一次性活动」** —— `The Hunt: Roblox 20` 拿了 80 分，但它 9/17–9/28 只有 12 天窗口，
