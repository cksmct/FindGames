...
>
> 🆕 **2026-09-26 四次修正：把「不适用」与「未测」分开显示**（用户问「上传代码应该触发更新，搜索量为什么还是 0」）
>
> 线上 3000 条实测：**有搜索量 36 条 · 来源型无搜索量但有官方量级 865 条 · 真未测 1746 条**。
> 「搜索量 0」在那 865 条上是**不适用**（名字从不上 Trends 上升/相关词榜；流量由「官方量级」承担 —— 其中很多条是**流量满分 30**），不是缺数据；1746 条才是真未测。页面现在把这两种情形分开写（`流量·搜索量` 行 + 整条的「未测」标记）。
> 同理「**动能·涨幅**」：它与搜索量**同源**（都来自关键词通道的绝对量），来源型条目拿不到时同样显示「**不适用**」（动能由「起飞档」承担）—— 不再显示「无」。
> 📌 **「0」必须能区分三件事：真的为零 / 不适用 / 没测到。** 这次是后两者混在一起了。
