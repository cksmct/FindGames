...
| 🎯 建站推荐 | 可做性分（0~100） | 逐维：**值 × 权重 = 贡献** → `Σ(值×权重) ÷ Σ权重(固定)` → `× 乘数` → **＋ 发现提前量加分(0~+12)**；**缺项按中性 50 计入固定分母**并在卡片标「参考」（缺哪几项也列出来） |
...
用户口径：「提前量可以说是加分项，没有就 0 分」。改的理由有实测支撑：它原是**权重最高**的一维（24/100），
但可算的 1525 条里 `lead>0` 只有 **7 条（0.5%）**、中位 **−4.6 天** —— 对绝大多数条目它只是恒定的低分拖累，没有区分度。

- 现在：`leadBonus = round(leadScore × 0.12)` → 100→+12 · 90→+11 · 80→+10 · 55→+7 · 35→+4 · 15→+2 · 5→+1；**无数据 / 上线超 1 年 → +0**；
- 加权维度从 7 项降到 **6 项**（`PICK_W` = demand 20 · surface 16 · comp 14 · momentum 8 · fresh 8 · quality 10 = 76）；
