...
export const SERP_RULES = {
  ...
  caveats: [
    ...
    "🛑 「专用站=0」不是无条件的好消息：**前十 6+ 家全是通用媒体 + 需求顶级（demand≥80）= 大作形态的饱和**（通用媒体只给有量的游戏写 codes 页）。前端对这种组合直接判「通用媒体已垄断」，不再当空位",
  ],
...
/**
 * ⚠️ **可选通道**：Brave Search API —— 稳定、有 JSON，但**要绑卡**（免费计划只验证身份、不扣款）。
 *
 * 🛑 2026-09-25 核实并更正（此前这里写的是"免费额度 2,000 次/月"，**那是过时信息，别再照着抄**）：
 *    官方现价 `Search $5 / 1,000 次请求`，所有套餐「每月 $5 免费额度」自动到账
 *    → 免费层实际约 **1,000 次/月**，且 **FAQ 明确要求绑定信用卡**。
 *    （网上"2,000 次免绑卡"是旧政策：2026-01 的博客仍那样写，2026-05 的记录已与官方页面一致。）
 *
 * 📊 本项目用量（2026-09-25 估算）：稳态 ≈ **780 次/月**（建站推荐 168 ÷ 7 天 TTL + 潜伏 25 ÷ 14 天 TTL
 *    ≈ 26 条/天）→ 占免费额度 **78%**，几乎没有余量。
 *    → 所以**默认不推荐换 Brave**：DDG 通道每轮约 2 条 × 24 轮 = **48 条/天**，已覆盖 26 条/天的稳态需求，
 *      代价只是**首次铺满约 4 天**（需求门槛之前是 20 天，方案 A 已把它压下来）。
 *    什么时候值得换：需实测条目涨到 300+，或首次铺满的时间变得不可接受。
 *
